...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-24 bg-[#000000] relative overflow-hidden">
      {/* Ambient Backdrops */}
      <div className="absolute top-1/3 left-10 w-[500px] h-[500px] bg-white/[0.015] rounded-full blur-[140px] pointer-events-none transform-gpu" />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-5xl font-bold text-white tracking-tight">
            Selected <span className="text-gradient-titanium">Engineering Projects</span>
          </h2>
          ...
          {/* Filter Pills */}
          <div className="flex items-center gap-2 mt-6 p-1.5 rounded-2xl bg-[#08080a] border border-white/10 shadow-xl shadow-black/80">
            <button
            ...
            >
              ...
              <div>
                ...
                <div className="flex items-center justify-between mb-4">
                  {project.featured ? (
                    <span className="inline-flex items-center gap-1.5 text-[11px] font-mono px-3 py-1 rounded-full bg-white/[0.08] text-white border border-white/20 shadow-sm">
                      <Star className="w-3.5 h-3.5 fill-emerald-400 text-emerald-400" /> Featured System
                    </span>
                  ) : (
                    <span className="text-[11px] font-mono text-zinc-300 bg-white/[0.04] px-2.5 py-0.5 rounded border border-white/10">
                      Project // {project.id}
                    </span>
                  )}
                  ...
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    {project.githubUrl && (
                      <a
                        ...
                        className="p-2 rounded-xl bg-[#08080a] text-zinc-400 hover:text-white border border-white/10 hover:border-white/25 transition-all"
                      >
                ...
                <p className="text-xs font-mono text-zinc-400 font-medium mt-1 mb-3">{project.tagline}</p>

                {/* Description */}
                <p className="text-zinc-300 text-xs sm:text-sm leading-relaxed mb-6 line-clamp-3">
                  {project.description}
                </p>

                {/* Metrics Preview if present */}
                {project.metrics && project.metrics.length > 0 && (
                  <div className="flex items-center gap-3 mb-6 p-2.5 rounded-xl bg-[#08080a] border border-white/10 font-mono text-xs">
              ...
              <div>
                <div className="flex flex-wrap gap-2 pt-4 border-t border-white/10">
                  {project.techStack.map((tech, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-lg bg-[#08080a] text-zinc-300 border border-white/10 text-[11px] font-mono group-hover:border-white/20 transition-colors"
                    >
                      {tech}
                    </span>
                  ))}
